...
RUN apt-get update && apt-get install -y build-essential git sudo vim zstd libzstd-dev && \
...
ENV C_INCLUDE_PATH=/usr/include/x86_64-linux-gnu:$C_INCLUDE_PATH
...
RUN mkdir -p /opt/slurm_sim_bld/slurm_sim_opt && \
    ...
    /opt/slurm_sim_tools/slurm_simulator/configure --prefix=/opt/slurm_sim \
        ...
        --enable-simulator && \
    sed -i 's|LIBCURL = -lcurl|LIBCURL = -L/usr/lib/x86_64-linux-gnu -lcurl|g' Makefile && \
    sed -i 's|LIBCURL_CPPFLAGS = -I/usr/include|LIBCURL_CPPFLAGS = -I/usr/include/x86_64-linux-gnu|g' Makefile && \
    make -j 8 && \
    make -j 8 install && \
