...
while true; do
    ...
    if [ "$CURRENT_LINES" -gt "$LAST_LINE" ]; then
        ...
        if echo "$NEW_CONTENT" | grep -q "JobId="; then
            ...
            FORMATTED_TIME=$(date -d "@$TIMESTART" +%Y%m%d_%H%M%S)
            DEST_FILE="$LOG_DEST_DIR/jobcomp_${FORMATTED_TIME}.log"

            echo "$NEW_CONTENT" > "$DEST_FILE"
            echo "Saved new jobs to $DEST_FILE (simulated time: $FORMATTED_TIME)"
